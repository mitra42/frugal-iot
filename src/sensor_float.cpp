...
Sensor_Float::Sensor_Float(const char* const moduleId, const char* const sensorId, const char * const moduleName, const char * const sensorName,  uint8_t width, float min, float max, float default_min, float default_max, const char* color, bool retain)
: Sensor(moduleId, moduleName, retain),
  output(new OUTfloat(moduleId, sensorId, sensorName, 0, width, min, max, default_min, default_max, color, false)), // Note id same as sensor id
  width(width) { 
    outputs.push_back(output);
  };

// Old form
Sensor_Float::Sensor_Float(const char* const id, const char * const name, uint8_t width, float min, float max, float default_min, float default_max, const char* color, bool retain)
  : Sensor_Float::Sensor_Float(id, id, name, name, width, min, max, default_min, default_max, color, retain)
{}
...
void Sensor_Float::setDefaultColor(const char* color) {
