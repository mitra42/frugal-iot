...
Sensor_Float::Sensor_Float(const char* const moduleId, const char* const leafId, const char * const moduleName,const char * const leafName, uint8_t width, float min, float max, float default_min, float default_max, const char* color, bool retain, uint8_t power3v3_pin, uint8_t power0v_pin)
: Sensor(moduleId, moduleName, retain, power3v3_pin, power0v_pin),
  output(new OUTfloat(moduleId, leafId, leafName, 0, width, min, max, default_min, default_max, color, false)), // Note id same as sensor id
  width(width) { 
