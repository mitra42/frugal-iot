...
class Sensor_Analog : public Sensor_Float {
  public:
    Sensor_Analog(const char* const moduleId, const char* const leafId, const char * const moduleName, const char * const leafName, const uint8_t pin, const uint8_t width, const float min, const float max, int offset, float scale, const char* color, bool retain, uint8_t power3v3_pin = 0xFF, uint8_t power0v_pin = 0xFF);
