...
void System_WiFi::connectInnerAsync(String ssid, String pw) {
  ...
  #endif
  WiFi.setHostname(frugal_iot.nodeid.c_str());
  // Disable modem sleep - on a weak/marginal link the periodic radio nap between beacons
  // (default power-save behaviour) makes it more likely to miss the handshake or drop out.
  // Cheap since these devices don't use Power_Deep/Power_Light modes that need the saving.
  #ifdef ESP8266
