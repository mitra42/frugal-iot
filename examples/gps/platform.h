...
#define SENSOR_GPS_RX_PIN 39       ; ESP32-S3 UART RX ← GNSS module TX // ESP32-S3 UART RX ← GNSS module TX#define SENSOR_GPS_TX_PIN 38       ; ESP32-S3 UART TX → GNSS module RX // ESP32-S3 UART TX → GNSS module RX#define SENSOR_GPS_0v_PIN 34   ; VGNSS_CTRL Passed as power0v_pin so Sensor::powerUp() drives it LOW // VGNSS_CTRL Passed as power0v_pin so Sensor::powerUp() drives it LOW#define SENSOR_GPS_WAKE_PIN 40 ; drive HIGH continuously to prevent module sleep: // drive HIGH continuously to prevent module sleep:#define SENSOR_GPS_RST_PIN 42 ; ; GNSS_RST must be driven HIGH to release the module from hardware reset: // ; GNSS_RST must be driven HIGH to release the module from hardware reset:#define SENSOR_GPS_DEBUG
// lib_deps =
//     ${common.lib_deps_lora_oled}
