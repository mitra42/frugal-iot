...
Frugal IoT - Agriculture Sensor with SHT (temp and humidity) DS18B30 (soil temp) and Soil moisture
...
Open `agri.ino`, then **Tools > Board** and pick the row for your hardware.
...
* **ESP32** - put the line `-DS2_MINI_5` in a file called `build_opt.h`,
  beside `agri.ino`. Despite the `.h`, that file holds **compiler flags, not
