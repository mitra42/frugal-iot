...
Frugal IoT - Google Sheets - largely abandoned and done on frugal-iot-logger
...
Open `gsheets.ino`, then **Tools > Board** and pick the row for your hardware.
...
* **ESP32** - put the line `-DLILYGO_T3_S3_SX127X_SHT` in a file called `build_opt.h`,
  beside `gsheets.ino`. Despite the `.h`, that file holds **compiler flags, not
...
The ESP8266 settings live in `esp8266/gsheets.ino.globals.h`, and that file has to be
moved up beside `gsheets.ino` before it is read. See `esp8266/README.md` - it is one
