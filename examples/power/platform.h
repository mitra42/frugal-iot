...
// S2_MINI, LIGHTWIFI_S2_MINI are alternative [env:] settings for one board - at most one.
#if (defined(S2_MINI) + defined(LIGHTWIFI_S2_MINI)) > 1
  #error "Define at most one of S2_MINI, LIGHTWIFI_S2_MINI - they are alternative settings for the same board, and defining two applies both."
#endif
// ===== [env:s2_mini] -> ARDUINO_LOLIN_S2_MINI, the DEFAULT for this board
#if (defined(ARDUINO_LOLIN_S2_MINI) && !defined(LIGHTWIFI_S2_MINI)) || defined(S2_MINI)
#define FRUGAL_IOT_BOARD_CONFIGURED
// platform = ${common.platform_esp32}
...
// ----- [env:lightwifi_s2_mini] also targets ARDUINO_LOLIN_S2_MINI
// Only one env per board can be active in the Arduino IDE, and [env:s2_mini]
// is the default. To use this one instead, define LIGHTWIFI_S2_MINI for the WHOLE
// build - it has to reach the library's sources too, so a #define in the .ino
// is not enough. Either:
//   put the flag -DLIGHTWIFI_S2_MINI (not a #define) in a file called build_opt.h
//     beside the .ino - it survives this file being regenerated - or
//   #define LIGHTWIFI_S2_MINI at the TOP of this file, which regenerating discards.
// To make it the default instead, and so need no define at all, set
//   custom_arduino_default = yes
// on [env:lightwifi_s2_mini] in platformio.ini (and remove it from any other env for
// this board), then re-run scripts/generate_platform_h.py.
#if defined(LIGHTWIFI_S2_MINI)
#define FRUGAL_IOT_BOARD_CONFIGURED
// platform = ${common.platform_esp32}
// board = lolin_s2_mini ; defines ARDUINO_LOLIN_S2_MINI
// build_flags = 
//     ${common.build_flags}
//     ${common.build_flags_powerlightwifi}
#define SYSTEM_OTA_SUFFIX "lightwifi_s2_mini"

// ==== ESP8266 boards ================================================

#endif // LIGHTWIFI_S2_MINI
