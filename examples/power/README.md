...
Frugal IoT - Power test suite - in progress
...
Open `power.ino`, then **Tools > Board** and pick the row for your hardware.

| Tools > Board | configuration | also define |
| --- | --- | --- |
| LOLIN C3 Pico | `c3_pico` |  |
| LOLIN S2 Mini | `s2_mini` |  |
| LOLIN S2 Mini | `lightwifi_s2_mini` | `LIGHTWIFI_S2_MINI` |
...
* **ESP32** - put the line `-DLIGHTWIFI_S2_MINI` in a file called `build_opt.h`,
  beside `power.ino`. Despite the `.h`, that file holds **compiler flags, not
...
The ESP8266 settings live in `esp8266/power.ino.globals.h`, and that file has to be
moved up beside `power.ino` before it is read. See `esp8266/README.md` - it is one
