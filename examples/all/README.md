...
Frugal IoT - Combination file - compile and link everything for testing
...
Open `all.ino`, then **Tools > Board** and pick the row for your hardware.
...
| ESP32 Dev Module | `ff_openmppt_ssd1327` |  |
...
* **ESP32** - put the line `-DLILYGO_T3_S3_SX127X_SHT` in a file called `build_opt.h`,
  beside `all.ino`. Despite the `.h`, that file holds **compiler flags, not
...
The ESP8266 settings live in `esp8266/all.ino.globals.h`, and that file has to be
moved up beside `all.ino` before it is read. See `esp8266/README.md` - it is one
