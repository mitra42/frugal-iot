...
Frugal IoT - Temperature and Humidity Sensor with SHT30
...
Open `remotedisplay.ino`, then **Tools > Board** and pick the row for your hardware.
...
* **ESP32** - put the line `-DS2_MINI_4X` in a file called `build_opt.h`,
  beside `remotedisplay.ino`. Despite the `.h`, that file holds **compiler flags, not
...
* **ESP8266** - add `#define D1_MINI_4X` at the top of
  `remotedisplay.ino.globals.h`, which the core force-includes into every source file.
...
The ESP8266 settings live in `esp8266/remotedisplay.ino.globals.h`, and that file has to be
moved up beside `remotedisplay.ino` before it is read. See `esp8266/README.md` - it is one
